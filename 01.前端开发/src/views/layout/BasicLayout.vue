          ...
          <template>
            <Dropdown trigger="click" @on-click="dropdown_click">
              ...
              <DropdownMenu slot="list">
                ...
                <DropdownItem name="logout">Exit</DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </template>
        </div>
        <div class="layout-role">
          <template>
            <Dropdown trigger="click">
              <a href="javascript:void(0)">Current role：{{ rolename }}</a>
            </Dropdown>
          </template>
        </div>
      </Header>
...
export default {
  data() {
    return {
      rolename: window.sessionStorage.rolename, //role
      username: window.sessionStorage.username, //username
    ...
    dropdown_click(name) {
      ...
      if (name == "logout") {
        ...
        if (token === "null" || token === "" || token === undefined) {
          that.$Message.success("Successfully log out ");
          that.$router.push({ name: "login" });
        } else {
          that.$Message.success("Failed to log out");
        }
      }
    },
    // Sets the navigation selection state
    setNavActive() {
      let that = this;
